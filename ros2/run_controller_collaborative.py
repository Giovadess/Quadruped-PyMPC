...
# from quadruped_pympc.helpers.passive_arm_interface import Passive_Arm_Int
...
SCHEDULER_FREQ = 200 # this is only valid if USE_SCHEDULER is True
...
class Quadruped_PyMPC_Node(Node):
    def __init__(self):
        ...
        # self.subscription_arm_state = self.create_subscription(Float32MultiArray, "arm_joint_positions" ,self.get_passive_arm_state_callback, 1)
        ...
        # self.arm_joint_positions = np.zeros(3)  # For the passive arm interface
            ...
            if time.time() - last_mpc_thread_time > 1.0 / MPC_FREQ:
                if(self.state_current is not None):
                    ...
                    self.nmpc_predicted_state = self.srbd_controller_interface.compute_control(self.state_current,
                                                                            ...
                                                                            self.optimize_swing)
                    # self.ref_state['ref_arm_position'] = np.array([0.,0.,0.])
                    # self.ref_state['ref_arm_velocity'] = np.array([0.,0.,0.])
                    # nmpc_GRFs, \
                    # nmpc_footholds, \
                    # nmpc_predicted_state, \
                    # _= self.controller.compute_control(
                    #                                     state_current,
                    #                                     ref_state,
                    #                                     contact_sequence,
    ...
    # def get_passive_arm_state_callback(self, msg):
    #     # msg is Float32MultiArray
    #     self.arm_joint_positions = np.array(msg.data)
    ...
    def compute_control_callback(self):
        ...
        ## for now lets print it 
        # print("Arm Joint Positions from Passive Arm Interface: ", self.arm_joint_positions)
        ...
        print(f"WBC dt={simulation_dt*1e3:.2f} ms  freq≈{1.0/simulation_dt:.1f} Hz")
        ...
        control_signal_msg.timestamp = self.last_mpc_loop_time #self.loop_time#self.last_mpc_loop_time
        ...
        trajectory_generator_msg.joints_position = np.concatenate([pd_target_joints_pos.FL, pd_target_joints_pos.FR, 
                                                                   pd_target_joints_pos.RL, pd_target_joints_pos.RR], axis=0).flatten()
